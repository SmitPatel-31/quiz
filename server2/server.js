...
// Emit the question and options to the players in the game
...
io.on('connection', (socket) => {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection('kahootGames').find(query).toArray(function(err, result){
                ...
                if(result[0] !== undefined){
                    ...
                    //The host is joining a room based on the pin
    ...
    socket.on('player-join-game', (data) => {
        ...
        if (player) {
            ...
            // Emit the question to the player
            var gameid = game.gameData.gameid;
            MongoClient.connect(url, function (err, db) {
                if (err) throw err;
                ...
                dbo.collection('kahootGames').find(query).toArray(function (err, res) {
                    ...
                    var question = res[0].questions[game.gameData.question - 1].question;
                    // console.log(question)
                    socket.emit('questionOnly', { q1: question });
